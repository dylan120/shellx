...
- 2026-06-02：用户反馈终端使用一段时间后“本来正常的渲染效果突然变了”，随后明确具体表现是本应彩色的终端文本突然变成无彩色渲染，并要求不要增加“刷新显示”手动操作入口。排查方向按 Electron/xterm.js 渲染状态处理：1）移除应用菜单中的 Chromium 缩放入口并在主进程锁定 `webContents` zoom level/factor，拦截 `Cmd/Ctrl +/-/0` 和 `zoom-changed`，避免误触页面缩放后 xterm 字体测量、列宽和视觉密度突变；2）在 xterm 层固定字体权重、禁用连字/kerning、保留 box drawing custom glyph 与重叠字形缩放，并恢复 `drawBoldTextInBrightColors`，避免依赖 bold+ANSI 的常见 `ls`/dircolors 输出发暗或像失去颜色；3）显式固定 ShellX 的 ANSI 16 色调色板，并只拦截 OSC 10/11/12/110/111/112 对默认前景、背景、光标的长期改写，不再拦截 OSC 4/104 调色板序列，避免远端程序正常维护 ANSI palette 时被吞掉；4）在标签切换、窗口重新激活、页面重新可见、DPR/visual viewport/终端容器尺寸变化后自动恢复 xterm theme、清理 texture atlas 并重绘当前内容。未添加任何“刷新显示”菜单或右键操作。验证：`npm run typecheck`、`npm run build`、`git diff --check` 通过；尚未在 macOS Electron 真机长时间运行、跨显示器/DPR 切换、远端 OSC 主题改写程序和真实 SSH TUI 中回归。
