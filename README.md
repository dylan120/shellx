...
- 当前 host key 确认依赖 `ssh-keyscan` 预扫描结果；应用会显式尝试拉取 `ed25519/ecdsa/rsa` 多种 host key 算法。若目标端口在 SSH 握手阶段直接断开或网络环境限制扫描返回，ShellX 会在错误中展示底层 `ssh-keyscan` 输出，方便区分端口错误、防火墙拦截和远端 `sshd` 异常
