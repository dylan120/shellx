...
- Electron 新版终端会固定 Chromium 页面缩放、显式固定 ANSI 16 色调色板，保护 ShellX 自身终端前景/背景/光标不被远端 OSC 序列长期改写，并在窗口重新激活、标签切换、DPR 或可视区域变化后自动恢复 xterm 渲染缓存，降低长时间使用后字体、颜色或贴图状态突然变化的概率。
