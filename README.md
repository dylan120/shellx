...
推送形如 `v0.2.0` 的 tag 后，GitHub Actions 会自动执行 `.github/workflows/release.yml`，复用 `scripts/build-dmg.sh --unsigned` 构建未签名的 `dist/ShellX-Release.dmg`，然后创建同名稳定 Release 资产。应用内更新检查只读取最新稳定 Release，因此不要把正式更新包发布为 draft 或 prerelease。
