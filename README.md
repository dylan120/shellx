...
- 支持账号密码认证；SSH 登录阶段的密码提示直接由终端中的系统 `ssh` 处理，ShellX 不再在连接 SSH 时读取或写入系统 Keychain
...
- 首次连接未知主机、主机指纹变更或服务器新增 host key 算法时都会展示指纹确认；私钥模式可启用 Keychain 集成；账号密码模式下 SSH 登录阶段的密码提示直接由终端中的系统 `ssh` 处理，不再由 ShellX 自动读取或注入 Keychain 密码
...
- 当前账号密码模式下，SSH 登录密码完全交由终端中的系统 `ssh` 处理；ShellX 不再在 SSH 连接阶段读取或写入 Keychain。若会话开启“保存到系统 Keychain”，当前仅影响需要独立密码回填的 SFTP 传输
...
- 账号密码不写入 `sessions.json`；若启用 Keychain 保存，当前仅用于 SFTP 传输时复用密码，不再用于 SSH 登录阶段的自动回填；若关闭保存或切换为非密码认证，会同步删除对应 Keychain 条目
