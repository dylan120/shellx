...
- 当前未知主机的 host key 确认依赖 `ssh-keyscan` 预扫描结果；应用会显式尝试拉取 `ed25519/ecdsa/rsa` 多种 host key 算法。若目标端口在 SSH 握手阶段直接断开或网络环境限制扫描返回，未知主机会继续展示底层 `ssh-keyscan` 输出；已存在 ShellX `known_hosts` 记录的主机会跳过预扫描，但实际 SSH/SFTP 连接仍使用 `StrictHostKeyChecking=yes` 由 OpenSSH 基于本地记录完成变更校验
