...
- 支持账号密码认证；当远端真正请求密码时，ShellX 会优先尝试从系统 Keychain 读取并自动注入，若没有可用密码再提示用户输入，并可在首次输入后写入系统 Keychain
...
- 首次连接未知主机、主机指纹变更或服务器新增 host key 算法时都会展示指纹确认；私钥模式可启用 Keychain 集成；密码模式在远端真正请求密码时会优先尝试读取系统 Keychain，若没有可用密码再提示用户输入，并可在首次输入后写入系统 Keychain 供后续连接复用
...
- 当前密码模式基于系统 `ssh` 进程的交互式密码提示自动注入，仅覆盖标准 `password:` 提示；ShellX 会在检测到远端密码提示后再尝试读取 Keychain 并自动回填，遇到自定义 challenge 或多因素认证场景时，可能仍需手动参与
...
- 账号密码不写入 `sessions.json`；若启用 Keychain 保存，会在首次成功输入连接密码后写入 macOS 登录 Keychain，后续在远端再次请求密码时优先自动读取；若关闭保存或切换为非密码认证，会同步删除对应 Keychain 条目
