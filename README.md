...
- 支持账号密码认证，密码仅保存在系统 Keychain，不写入 `sessions.json`
...
- 支持基础 `lrzsz` 双向文件传输触发
- 首次连接未知主机、主机指纹变更或服务器新增 host key 算法时都会展示指纹确认；私钥模式可启用 Keychain 集成；密码模式会从系统 Keychain 读取并在 SSH 密码提示出现时自动注入
...
- 当前密码模式基于系统 `ssh` 进程的交互式密码提示自动注入，仅覆盖标准 `password:` 提示，不保证所有自定义 challenge/多因素认证场景
...
- 账号密码不落盘，统一保存在 macOS Keychain；若删除会话或切换为非密码认证，会同步删除对应 Keychain 条目
